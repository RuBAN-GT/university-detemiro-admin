# Сервер аутентификации для Detemiro

Данный бандл использует ядро [Detemiro](https://github.com/RuBAN-GT/detemiro) и позволяет сформировать модульной сервер аутентификации.

> * Для начала работы необходимо отредактировать путь до ядра в index.php.
> * Пример клиента и его документация находится в директории *client*.
> * В *det-content/modules* представлен модуль *example* в качестве примере модуля аутентификации.

## Дополнительные требования

* СУБД MySQL/PostgreSQL
* Кеш-сервер Redis/xcache/Memcache
...
.mapping   | Ассоциативный массив подмены ключей (оригинальный ключ => новый ключ). Если указан, будут использоваться только его ключи.
...
auth.checkQuery          | Зона для дополнительной проверки запроса клиента              | (array) запрос клиента, (array) параметры сервиса        | 
